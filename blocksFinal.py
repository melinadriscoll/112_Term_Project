...
def init(data):
    ...
    #example tilting blocks
    data.exampleBlocks = [[data.width//2,data.height-data.margin-80,
    data.width//2+80,data.height-data.margin]]
    data.exampleMovingBlocks = []
...
def createBlock(data):
    ...
    if data.game2BlockState:
        data.exampleMovingBlocks.append([data.xCoor1,data.yCoor1,data.xCoor2,
        data.yCoor1,data.xCoor2,data.yCoor2,data.xCoor1,data.yCoor2])
        ...
        if data.currBlock != None:
            ...
            if event.keysym == "Down":
                ...
                if data.currBlock[2] > data.width-data.margin:
                    ...
                    data.currBlock[2] -= data.positionShift
    # if data.game2BlockState:
    #     for block in data.exampleMovingBlocks:
    #         block[1] += 1
...
def timerFired(data):
    ...
    if data.game2BlockState:
        if data.count%100 == 0:
            createBlock(data)
        if data.count > 0 and data.count%10 == 0:
            for block in data.exampleMovingBlocks:
                exampleMoveBlock(data,block)
        data.count += 1
            ...
            if block[3] == stoppedBlock[1]:
                ...
                stoppedBlock[0] < block[2] < stoppedBlock[2]:
                    ...
                    if isBalanced(data, data.nowBlock, stoppedBlock):
                        ...
                        data.movingBlocks.remove(block)
                        continue
                    if isBalanced(data,data.nowBlock,stoppedBlock) and \
                    data.nowBlock[-1] == "littleright" or \
                    data.nowBlock[-1] == "littleleft":
                        print("hi")
                        for stackBlock in data.stacks:
                            print(stackBlock)
                            if (stackBlock[0] < data.nowBlock[0] < \
                            stackBlock[2]) or (stackBlock[0] < \
                            data.nowBlock[2] < stackBlock[2]):
                                stackBlockCenter = (stackBlock[0] + \
                                stackBlock[2])//2
                                nowBlockCenter = (data.nowBlock[0] + \
                                data.nowBlock[2])//2
                                if len(stackBlock) == 1:
                                    stackBlock.append(nowBlockCenter-stackBlockCenter)
                                else:
                                    stackBlock[1] += nowBlockCenter-stackBlockCenter
                        print(data.stacks)
                    else:
            ...
            if block[3] == bottomBlock[1]:
                ...
                bottomBlock[0] < block[2] < bottomBlock[2]:
                    ...
                    if isBalanced(data, data.nowBlock, bottomBlock):
                        ...
                        data.movingBlocks.remove(block)
                        continue
                    if isBalanced(data,data.nowBlock,bottomBlock) and \
                    data.nowBlock[-1] == "littleright" or \
                    data.nowBlock[-1] == "littleleft":
                        for stackBlock in data.stacks:
                            print(stackBlock)
...
def isBalanced(data, fallingBlock, placedBlock):
    ...
    #if it is already the tipping block
    if data.nowBlock == fallingBlock and not(isinstance(data.nowBlock[-1],str)):
        #if block is off the right side of the underlying block by more than
        #half of its size
        if fallingBlock[0] > placedBlock[0]:
            if fallingBlock[0] - placedBlock[0] > fallingBlockSize//2:
        ...
        #half of its size
        if fallingBlock[0] < placedBlock[0]:
            if placedBlock[0] - fallingBlock[0] > fallingBlockSize//2:
...
def exampleMoveBlock(data,newblock):
    newblock[1] += 10
    newblock[3] += 10
    newblock[5] += 10
    newblock[7] += 10
    if newblock[5] >= data.height - data.margin:
        newblock[5] = data.height - data.margin
        newblock[7] = data.height - data.margin
    else:
        for exblock in data.exampleBlocks:
            if exblock[1] <= newblock[5]:
                if exampleBalanced(data,newblock,exblock):
                    data.exampleBlocks.append(newblock)
...
def redrawAll(canvas, data):
    ...
    if data.game2BlockState:
        canvas.create_rectangle(data.width//2,data.height-data.margin-80,
        data.width//2+80,data.height-data.margin,fill="peachpuff")
        for block in data.exampleMovingBlocks:
            canvas.create_polygon(block[0],block[1],block[2],block[3],block[4],
            block[5],block[6],block[7],fill="peachpuff")
